...
@Component({
  selector: 'ngx-agregar-cardex',
  templateUrl: './agregar-cardex.component.html',
  styleUrls: ['./agregar-cardex.component.scss']
})
export class AgregarCardexComponent implements OnInit {
  ...
  cerrar_registrar (dialog: any) {
    dialog.close ();
    this.productos_para_seleccionar = [];
  }
  ...
  registrar (items: any [], ref: any) {
    items.forEach ((i: any) => {
      if (i.cantidad >= 1 && i.cantidad <= i.ref.stock && i.cantidad !== undefined && i.cantidad !== null) {
        this.items.push ({
  ...
  crear_ruta () {
    ...
    this.directionsService.route (request, (response: any, status: any) => {
      if (status == 'OK') {
        ...
        for (let index = 0; index < response.routes [0].legs.length - 1; index++) {
          this.clientes_picking [index].distancia = response.routes [0].legs [index].distance.value;
          this.clientes_picking [index].duracion = response.routes [0].legs [index].duration.value;
        }
  ...
  submit () {
    if (confirm ('Crear?')) {
      ...
      let cardex: any = {
        ...
        hora: moment (this.form.value.fecha).format ("HH[:]MM"),
        hora_fin_carga: null,
        hora_fin_ruta: null,
      ...
      this.database.add_cardex (cardex, informacion_clientes, Productos)
        .then (() => {
  ...
  validar_limite (event: any, item: any) {
    if (event > item.ref.stock) {
      item.cantidad = item.ref.stock;
    }
  }
}
